...
function ts_get_used_googe_web_fonts()
{
	...
	if (count($fonts_to_get) > 0)
	{
		...
		{?>
			<link href="<?php echo $protocol; ?>://fonts.googleapis.com/css?family=<?php echo urlencode($font);?>:400,800,300,700<?php echo $subset; ?>" rel="stylesheet" type="text/css">
		<?php
		}
	}
	return $fonts_return;
...
function ts_show_preheader() {	
	...
	if ($show_preheader != 'no' && is_array($preheader) && count($preheader) > 0): ?>
				...
				foreach (array('left','right') as $float):
					foreach ($preheader as $item):
						...
						<div class="<?php echo $float; ?>">
							...
							<?php if ($item['icon'] != 'no'): ?>
								<i class="<?php echo $item['icon']; ?> <?php echo $item['icon_position']?>"></i>
							<?php endif;
